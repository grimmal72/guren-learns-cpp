...
enum Command {
    ...
    Look, Map, Inventory, Help, Wait, Quit,
    Unknown
};
...
Command parseCommand(const std::string& word) {
    ...
    if (word == "wait") return Command::Wait;
    if (word == "quit" || word == "q") return Command::Quit;
    return Command::Unknown;
}

int main() {
    std::cout << "HAUNTED - A HORROR GAME\n";
    std::cout << "/////////////////////////////////\n\n";
    ...
    while (true) {
        ...
        if (parts.empty()) {
            ...
            switch (cmd) {
            case Command::North:
                if (currentRoom == MANSION_ROOF_LEFT) {
                    if (roofGateUnlocked) {
                        std::cout << "You go through the gate to the helipad.\n\n";
                        if (tryMove("north")) {
                            moves++;
                        }
                    }
                    else {
                        if (takeKeyInputted == true) {
                            std::cout << "There's a locked gate on the way to the helipad, but you have the key to it. You open it. It creaks open. Almost to safety now.\n\n";
                            roofGateUnlocked = true;
                            // Note: doesn't move this turn -- unlocking the gate takes the whole action.
                            // Player needs to type "north" again next turn to actually walk through.
                        }
                        else {
                            std::cout << "There is a locked gate leading to the helipad. I guess it's to keep the helicopter safe? Anyways, you'll need to find a key somewhere in the mansion.\n\n";
                        }
                    }
                }
                else {
            ...
            case Command::Inventory:
                ...
                break;
            case Command::Help:
                std::cout << "\nHELP\n----------------------------------\nThis is a room to room text adventure. For simplicity's sake, the rooms are organized by north, east, south, and west from each other. Input one of these directional words, such as \"north\", \"n\", \"south\", \"s\", \"w\", etc, as a command, to move in that direction to a room (if it exists). You can also type \"go north\", for example.\n\nTo look at the room description again, type \"look\".\n\nTo look at a specific object in the room, such as a window, type \"look at window\", \"look towards the window\", \"look up at window\", or anything else that begins with the verb and ends with the subject. If there's multiple subjects, the meaning might not be processed correctly.\n\nYou are trapped in a mansion surrounded by zombies. If you roam around for 100 turns without escaping, they will break in and eat you. Input \"moves\" to see how many moves you have taken. This is based on room changes, not any commands/actions you have made inside a room.\n\nHowever, if you're morbid, you can intentionally wait for your own demise. Input \"wait\" to wait the equivalent of 10 moves worth of time. That's all that wait does. You can also see how many game overs you've gotten by inputting \"restarts\".\n\nInput \"take\" followed by the item to take the item. Well, if it's a takeable item, that is. If it's takeable, the console will tell you you picked it up.\n\nThere is a map in this game. If you've found it and took it, type \"map\" to look at it.\n\nThere's an inventory which can hold one weapon at a time, as well as the map and the key. Type \"inventory\" or \"i\" to view the inventory. Type \"drop\" to drop a weapon if you want to replace it with a different one.\n\n";
                break;
            case Command::Wait:
                moves += 10;
                // Waste 10 moves, and print the following statement.
                std::cout << "You wait around a bit. The cellular reception is out. Why does nothing work right now? You try to remember how you got here, but everything is fuzzy. Some scary flashbacks spring to mind. Do they have something to do with this situation? Hmm, better keep going.\n\n";
                break;
            case Command::Quit:
                std::cout << "Goodbye.\n";
                return 0;
            ...
            if (verb == "look") {
                ...
                if (wasFound) {
                    ...
                    std::cout << "You can't have a conversation with that! That's silly!\n";
                }
            }
            else if (verb == "take") {
                if (subject == "chainsaw") {
        ...
        // Various game over checks.
        if (currentRoom == HEDGE_LABYRINTH) {
            return 0;
        }
        else if (currentRoom == TOP_OF_RIGHT_SPIRE) {
            return 0;
        }
        else if (currentRoom == HELIPAD) {
            std::cout << "You win! You escaped the mansion!\n";
            std::cout << "Thank you for playing!\n\n";
            return 0;
        }

        else if (currentRoom == FRIDGE) {
            if (fridgeZombiesDefeated != true) {
                std::cout << "There's 4 zombies in here!\n";
                if ((currentWeapon == CHAINSAW) || (currentWeapon == KNIFE)
                    || (currentWeapon == WIRE) || (currentWeapon == RAKE)
                    || (currentWeapon == SWORD)) {
                    // put up a fight
                    if (currentWeapon == CHAINSAW) {
                        std::cout << "You rev up your chainsaw. It roars, echoing through the kitchen and fridge. You gratuitously demobilize the zombies, and they fall to the floor.\n\n";
                        fridgeZombiesDefeated = true;
                    }
                    else if (currentWeapon == KNIFE) {
                        std::cout << "You try to attack one of the zombies with your knife, but the other three bite into you as you lunge. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == RAKE) {
                        std::cout << "You try to hit the zombies with your dinky little rake but it does nothing!  They push you over and eat you! Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == WIRE) {
                        std::cout << "You're taken by surprise by them, so you don't have time to use your piano wire to elaborately set a tripwire for the zombies. As you kneel down to do so, they swarm you and bite into your neck, spraying blood everywhere. Game over!\n\n";
						return 0;
                    }
                    else if (currentWeapon == SWORD) {
						std::cout << "You heave your sword back, and slash through all four of them. A shwing sound resonates as the zombies fall to the ground, grave incisions made.\n\n";
                        fridgeZombiesDefeated = true;
                    }
                }
                else {
                    std::cout << "You're unarmed! You try to run but they swarm after you and rip you apart! Game over!\n\n";
                    return 0;
                }
            }
        }
        ...
        if (currentRoom == MASTER_BEDROOM) {
            if (bedroomKnightDefeated != true) {
                if ((currentWeapon == CHAINSAW) || (currentWeapon == KNIFE)
                    || (currentWeapon == WIRE) || (currentWeapon == RAKE)
                    || (currentWeapon == SWORD)) {
                    std::cout << "Heaving as you enter the room, to your surprise, a seven foot tall knight with a sword starts menacingly walking towards you from the corner of the room.\n\n";
                    // put up a fight
                    if (currentWeapon == CHAINSAW) {
                        std::cout << "You rev up your chainsaw and attempt to slash the knight, but your chainsaw just bounces off of its metal armor, doing nothing. The knight impales you with its sword, then throws you aside. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == KNIFE) {
                        std::cout << "You try to stab it with your knife, but it just slides right off of its armor, doing nothing. The knight slashes you with its sword, cutting you in half. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == RAKE) {
                        std::cout << "You try to hit it with your dinky rake but it does nothing! The knight slashes you with its sword, cutting you in half. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == WIRE) {
                        std::cout << "You quickly rummage through your pockets, your heart racing. You find the piano wire! You tie it between the balcony railing and the bed leg, forming a tripwire. The knight trips on it, stumbles, and slashes towards you, but misses, falls to the ground, and shambles to pieces. There was nothing in the armor. It must have been possessed.\n\n";
                        bedroomKnightDefeated = true;
                    }
                }
                else {
                    std::cout << "You're unarmed! The knight walks menacingly toward you. It impales you with its sword, then tosses you aside. Game over!\n\n";
                    return 0;
                }
            }
        }

        if (currentRoom == PATH_TO_HELIPAD) {
            if (helipadMonsterDefeated != true) {
                std::cout << "A large zombie jumps onto the metal leading up to the helipad. It seems to be more powerful than the other zombies, it jumped 40 feet!\n\n";
                if ((currentWeapon == CHAINSAW) || (currentWeapon == KNIFE)
                    || (currentWeapon == WIRE) || (currentWeapon == RAKE)
                    || (currentWeapon == SWORD)) {
                    // put up a fight
                    if (currentWeapon == CHAINSAW) {
                        std::cout << "You rev up your chainsaw, but it just swats you off the roof. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == KNIFE) {
                        std::cout << "You try to impale its foot with your dinky knife, but it just swats you away, and you fly off of the roof. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == RAKE) {
                        std::cout << "You try to hurt its foot with your dinky rake, but it just swats you away, and you fly off of the roof. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == WIRE) {
                        std::cout << "You try to set a tripwire for the zombie using your piano wire, but it doesn't work. The monster is so strong it can just walk right through it. It walks over to you and swats you off of the roof. Game over!\n\n";
                        return 0;
                    }
                    else if (currentWeapon == SWORD) {
                        std::cout << "It lunges at you, but you slash it with your sword. It collapses to the ground, and slips off the roof. You've defeated it.\n\n";
                        helipadMonsterDefeated = true;
                    }
                }
                else {
                    std::cout << "You're unarmed! The monster walks over and swats you off of the building! Game over!\n\n";
                    return 0;
                }
            }
        }
