    ...
    while (true) {
        ...
        if (parts.empty()) {
            ...
            if (verb == "look") {
                ...
                // objects.end() is the place after the map ends, in kind of an out-of-bounds area. So if the iterator actually
                // found something first, then it won't try to attach onto end(), the out-of-bounds area of the map.
                ...
                if (subject == "weapon") {
                    // Generic "drop weapon" always works if you're holding any weapon.
                    if (hasWeapon()) {
                    ...
                    // Covers chainsaw / knife / rake / sword / wire.
                    std::string weaponName = weaponToString(currentWeapon);
                    dropWeapon();
                    std::cout << "You dropped the " << weaponName << ".\n";
                }
                    ...
                    // The user typed a real weapon name, but they're not holding it.
