...
// Example of an enum being used:
    ...
    // Example of syntax to choose what the enum constant will be set to.
    // GameState currentState = MENU;

    // What does this mean? It means we can use a "list" of characters, and then choose who
    // the currently selected one is.
...
void whoIsSpeaking(int dialogue, SDL_Color black, TTF_Font* font, std::string text, SDL_Renderer* renderer) {
    ...
    SDL_Surface* speakerNameTextSurface =
    ...
    SDL_Texture* speakerNameTextTexture =
        SDL_CreateTextureFromSurface(
            renderer,
            speakerNameTextSurface
        );

    SDL_Rect speakerNameTextRect = { 100, 450, 0, 0 };

    SDL_QueryTexture(
        speakerNameTextTexture,
        NULL,
        NULL,
        &speakerNameTextRect.w,
        &speakerNameTextRect.h
    );
    ...
    SDL_DestroyTexture(speakerNameTextTexture);
};
...
int main(int argc, char* argv[])
{
    ...
    while (running)
    {
        ...
        whoIsSpeaking(dialogue, black, font, text, renderer);
